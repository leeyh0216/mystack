...
EMR client는 public Proxy에 AWS JSON 1.1 요청을 보냅니다. EMR inbound adapter가 고정 service model을
...
`RunJobFlow`는 local cluster model을 만들고 `AddJobFlowSteps`는 순서가 있는 Step을 추가합니다. Read 호출은
...
runnable Step은 정확한 argument vector를 해석하고 local Spark child process를 시작합니다. Argument를
...
취소는 child shutdown을 요청한 뒤 설정 deadline 안에서 escalation하며 runtime close는 queued task와
artifact client, driver lock을 정리합니다.

<!-- section: logs -->
## 로그, 종료, local 제약

Live stdout/stderr는 EMR management read model에서 볼 수 있습니다. `LogUri`가 있으면 terminal controller,
syslog, stdout, stderr, local-driver stream을 해당 S3 prefix에 게시합니다. Step 결과와 로그 게시 retry는
...
이는 EC2/YARN/HDFS emulator가 아닌 local-mode Spark입니다. instance count는 worker를 만들지 않는 요청
metadata이며 Spark History Server와 종료된 live UI 복원은 제공하지 않습니다. Console은 신뢰하는 local
