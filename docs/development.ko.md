...
AWS 엔드포인트 환경변수는 [공식 SDK endpoint 설정](https://docs.aws.amazon.com/sdkref/latest/guide/feature-ss-endpoints.html)을 따릅니다.
...
`Clone Repository in Container Volume` 대신 Host의 로컬 clone을 `Reopen in Container`로 여세요.
...
1. executable `--config`, `MYSTACK_CONFIG_FILE`, 기본값 순으로 YAML 파일 선택
...
Timeout은 YAML `tests` section에서 읽습니다. Service process/bootstrap 제한 시간은 별도이며 가능한 경우 바깥 테스트 제한 시간보다 먼저 어댑터가 hung subprocess를 종료합니다. Annotation collector는 `tests.compatibility_collection_timeout_seconds`로 제한한 별도 child process이며 테스트를 실행하지 않습니다.
...
지정하면 명시적인 기본 10초 테스트/hook deadline을 바꿀 수 있습니다. `make pre-commit`은
`uv.lock`과 `package-lock.json`으로 재현되는 repository-로컬 hook을 설치하고 실행합니다.
Python과 React/TypeScript lint/format, 한·영 문서, container requirement lock, botocore 모델 manifest와 생성된
상호운용성·오류 근거의 변경을 commit
전에 차단합니다. Hook lifecycle은 공식 [pre-commit 설치·사용
계약](https://pre-commit.com/#install)을 따릅니다.

Compatibility 시나리오는 실제 `contract` 또는 `e2e` 테스트 옆의 type이 있는 pytest annotation으로
선언합니다. `make compatibility-evidence-generate`는 collection만 실행하고 CI 표와 한·영 근거를
만들며 테스트 body를 실행하지 않습니다. `make compatibility-evidence-check`는 marker, lock/실행 환경,
modeled API 작업, API 근거 차이를 거부합니다. `make compatibility-check`는 생성 표와 명시적 scope policy도 검증합니다.
`CompatibilityProfile.expected_duration_minutes`는
생성한 GitHub Actions job 시간 상한이며 로컬 collection deadline이나 pytest 제한 시간이 아닙니다. 일반 Python CI job은 전체 계약 module을
실행 전에 원본 SHA, lock/config, platform, producer run, 파일 digest를 담은 frontend bundle manifest를 검증합니다.
Docker E2E와 release preflight도 같은 bundle을 검증해 재사용하며, 산출물가 없으면 로컬 build로 전환하고
stale/corrupt 산출물는 거부합니다. browser E2E는 독립 실행 가능하게 유지합니다. Artifact lifecycle은 공식
...
- Wire 메타데이터/공통 JSON 직렬화: `shared/src/mystack/aws_protocol`
...
- EMR UI application/DTO/API 조립: `emr/ui`; Glue UI application/DTO/API 조립: `glue/ui`;
  서비스 UI는 `@mystack/ui`만 import하며 서로를 import하면 안 됨
- 안정적인 공개 UI forwarding만 담당: `proxy/src/mystack/proxy/forwarder.py`; Proxy는 서비스
  React code를 package하거나 서비스 DTO를 알면 안 됨
...
- Inbound use case의 최소 Protocol: 각 서비스의 `application/use_cases.py`
- AWS API 작업 mapping: 각 서비스의 `adapters/inbound/aws_{family}.py`; 검토한 구현 inventory:
...
import를 바꾼 뒤 `make architecture-check`를 실행하세요. 실패 결과에서 원본, imported module,
위반 rule, 수정 안내를 확인할 수 있습니다.

<!-- section: troubleshooting -->
## 문제 해결

- `make bootstrap`: 도구, 문서, 모델 drift, fast 테스트 문제 진단
- `make logs SERVICE=proxy`: JSON 경계 event 확인
- `make threads`, `make tasks`: frame locals 없는 live stack 수집
- `model-drift-report.json`: 변경 API 작업과 수정 위치
- `api-coverage-drift-report.json`: 미분류·삭제·데이터 구조 변경·잘못 분류된 작업과 수정 경계
- 호환성 실패 log: case ID, 정확한 version, 시나리오 ID, 모델 fingerprint, 검증 정보 hash와 수정
  안내. 공식 출처와 테스트 annotation을 검토한 뒤 `make compatibility-evidence-generate`와
  `make compatibility-evidence-check` 실행
- E2E 산출물: 모든 container log
