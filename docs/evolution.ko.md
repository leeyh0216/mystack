...
- EMR/Glue operation shape 또는 의미 변경은 해당 서비스 inbound adapter와 use case에서 처리합니다.
...
2. report는 metadata 변경과 추가·삭제·shape 변경 operation을 구분합니다.
3. CI 로그는 pinned/current version, model fingerprint, operation, input shape, `fix_hint`를 포함합니다.
