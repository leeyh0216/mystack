...
3. Run `make compatibility-evidence-generate` and review the CI/local report in
   `ci-artifacts/compatibility/`. The generator uses [pytest collection](https://docs.pytest.org/en/stable/how-to/usage.html), so
...
5. Also run `make compatibility-check`. Keep typed pytest compatibility annotations and
   `contracts/compatibility-scope-policy.yaml` aligned; do not alter the independent Glue
   error-condition policy unless the API behavior itself changes.
...
verification, or changed case selection. A case records exact versions, scenario/API-operation IDs,
model fingerprints, and a deterministic digest so logs point maintainers at the broken
boundary.
