...
| Spark + Iceberg + Glue Catalog | 세로 경로 구현: Iceberg 1.7.1 create/append/read/schema evolution, 원자적 `VersionId` pointer commit과 concurrent-writer retry E2E | Partition/sort evolution, row-level DML, snapshot/ref/procedure, lifecycle operation과 더 넓은 Iceberg API |
...
protocol](protocols/glue-iceberg-commits.ko.md)을 참고하세요. `GetPartitions`는 type이 있는 key, 우선순위,
