...
서로 다른 최상위 import 이름은 같은 제품의 소유 관계를 드러내지 못하고 새 서비스가 관련 없는
...
initializer를 추가하면 함께 설치할 때 조용히 깨질 수 있으므로 원본 tree import만으로는
충분히 검증할 수 없습니다. Ruff `TID252`가 로컬, pre-commit, CI에서 모든 상대 import를
거부합니다. Test는 다른 테스트 module을 import하지 않고 자체 테스트 data를 구성하거나 소유자가
...
`tests.package_smoke_timeout_seconds`에서 읽습니다. Architecture 테스트도 namespace root initializer를
