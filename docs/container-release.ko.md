...
`config/release/registry-release.json`이 component name, Dockerfile, platform, scan 정책, 제한 시간, 정확한 tag
pattern, snapshot 보존 기간을 관리합니다. `latest`는 게시하지 않습니다.

<!-- section: contract -->
## 게시 계약

- PR과 feature event는 build/테스트만 수행하며 package permission이 없습니다.
...
- Registry login 전에 설정된 component/platform별 로컬 build와 scan을 수행합니다. Content hash가
...
- Release를 마무리하기 전에 익명 runner가 게시한 Glue platform manifest digest를 각각 찾고 정확한
  `image@platform-digest`에 상한이 있는 `--verify-sqlite-runtime` command를 실행합니다. 이 보고서는
  앞선 로컬 build 결과만 믿지 않고 산출물에서 원본-built SQLite version, 검증한 WAL 경로,
  실행 환경 architecture를 보여줍니다.
- 고정 Trivy는 공식 [image 명령
  계약](https://trivy.dev/docs/latest/guide/references/configuration/cli/trivy_image/)을 따릅니다. 모든
  job, scan, 테스트, 외부 명령에는 명시적 제한 시간이 있습니다.
...
version PR** 워크플로 또는 `versioning.ko.md`의 명령을 사용합니다.
...
공개 전환을 되돌릴 수 없다고 안내합니다. 자동화는 이 작업을 의도적으로 수행하지 않습니다.
세 package가 모두 공개이 될 때까지 워크플로의 익명 검증이 실패합니다.
...
공개 container package의 익명 접근을 설명합니다. Visibility 실패를 숨기기 위해 consumer token을
추가하지 않습니다.

<!-- section: vulnerability -->
## 취약점과 rollback 의미

이 프로젝트는 GHCR에서 ECR 방식 scan-on-push 계약을 사용하지 않습니다. Registry 인증 전에 로컬
platform image를 검사합니다. 검증, build, 제한 시간, 근거 누락, 설정된 취약점 중 하나라도 실패하면
authorization을 만들지 않습니다. 진단용 raw `preflight-*` 산출물는 남습니다.

Push 뒤 Glue 실행 환경 probe에도 상한을 둡니다. Nonzero exit, 제한 시간, 잘못된 보고서가 발생하면
release를 멈추기 전에 published-release 검증 정보 산출물 아래에 실패한 `sqlite-runtime.json`을
작성합니다. Document에는 최대 4,096자인 redacted stdout/stderr tail만 남기며 전체 container output,
token, credential, query value는 넣지 않습니다. Probe가 실패해도 검증 정보 upload를 수행하고 14일간
보존합니다.

취약점 판정 기준은 기본적으로 설정 severity를 모두 거부합니다. 알려진 upstream 실행 환경 finding은
...
authorization 산출물를 명시적이고 결정적으로 유지하기 위해 Mystack이 파일 기반 coordinate를 직접
평가합니다.

현재 예외는 고정 Spark 3.5.4/EMR 7.8과 AWS Glue 5 프로필에 포함된 dependency만 다룹니다. Glue
image에서는 scan 전에 Job 전용 Maven cache, Delta, Hudi, Flink, streaming, Redshift, 중복 PySpark JAR
surface를 제거합니다. 두 로컬 emulator 모두 Derby LDAP authentication과 ZooKeeper quorum 서비스를
...
있으며, 검증 가능한 수정 실행 환경을 고정할 때까지 Glue 입력은 신뢰할 수 있는 로컬 dataset이어야
...
page](https://zookeeper.apache.org/security/)를 정책에 연결했습니다. Maintainer는 실행 환경을 patch하거나
...
메타데이터에는 30일 보존 목표를 기록하지만 삭제는 아직 자동화하지 않습니다.
...
| `registry.gate.verify.*` | 전체 근거 set 검증 | 누락/추가 산출물, 원본 SHA, version 또는 scan 결과 |
...
| `glue.sqlite.preflight.failed` | 게시한 Glue platform digest가 SQLite 실행 환경을 증명하지 못함 | 보존한 redacted `sqlite-runtime.json`, Dockerfile dependency resolver, 실행 환경 policy 확인 |
