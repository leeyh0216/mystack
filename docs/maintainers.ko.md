...
3. [프로젝트 기준선](project/baseline.ko.md)과 [구현 기반 UseCase](project/usecase-catalog.ko.md)로
   현재 구현 상태를 확인합니다.
4. `make test`, `make contract`, 필요한 Docker E2E를 명시적 제한 시간으로 실행합니다.
...
- Version이 고정된 upstream 어댑터: [ADR-0002](adr/0002-versioned-upstream-adapters.ko.md)
- 공통 PEP 420 import namespace: [ADR-0003](adr/0003-pep420-namespace-packages.ko.md)
- 새 서비스를 Proxy에 등록: [Proxy 확장 안내](extending-proxy.ko.md)
...
- Iceberg partition/schema/sort/identifier 보장과 클라이언트 변경 수정 위치: [Iceberg evolution
...
- Iceberg time travel, ref, 메타데이터/maintenance procedure, S3 영향과 수정 위치: [Iceberg
...
- AWS Open Table Format 요청 구조, 서비스 소유 Iceberg 메타데이터, S3 보상과 수정 위치:
...
- 고정 botocore 모델과 구현 상태: [API coverage](compatibility/api-coverage.ko.md)
- 외부 클라이언트별 E2E claim: [Client 호환성 표](compatibility/client-matrix.ko.md)
- AWS, boto, Spark 변경 대응 위치와 자동 검사: [변경 대응 정책](evolution.ko.md)

Protocol을 변경할 때는 controller만 보지 말고 모델 manifest, dispatcher, domain error mapping,
공개-Proxy 계약와 E2E를 같은 issue에서 갱신합니다.
...
- Unit, architecture, 결정적 계약, Docker E2E: [Test 전략](testing.ko.md)
- Pull request와 scheduled 워크플로: [CI 안내](ci.ko.md)
- 모든 테스트 명령은 `config/runtime/mystack.yaml`의 제한 시간 또는 명시적 `--timeout`을 사용합니다.
- Protocol이나 클라이언트 호환성 수정은 최소 unit/계약와 실제 공개-Proxy E2E 증거를 함께
  추가합니다.
...
2. 한 issue의 code, 테스트, user/maintainer 문서를 함께 완성합니다.
