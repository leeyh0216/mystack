...
- [설정 file mount](#설정-file-mount)
- [Upgrade, rollback, 정리](#upgrade-rollback-정리)
- [공식 참고 자료](#공식-참고-자료)
<!-- toc:end -->

실행 중인 stack을 확인하고, service가 소유하는 UI를 사용하고, 설정 file을 mount하거나, 로컬
...
Glue UI에서 database, table, schema, partition, parameter, raw metadata를 탐색합니다.
...
확인합니다. Request나 subprocess가 멈춘 경우에는 service log와 진단 endpoint를 사용합니다.
Event field와 문제 해결 경계는 [관측성 안내](observability.ko.md)에 있습니다.

<!-- section: configuration -->
## 설정 file mount

실행할 image version과 같은 configuration 및 Compose overlay를 내려받고 모든 Mystack service에
read-only로 mount합니다.
...
Mount한 file을 변경한 뒤에는 영향을 받는 service를 재시작합니다. Setting 이름, override 우선순위,
timeout 값은 [설정 reference](configuration.ko.md)를 참고하세요.
