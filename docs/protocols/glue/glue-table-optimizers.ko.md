...
`BatchGetTableOptimizer`, `ListTableOptimizerRuns` 여섯 API 작업을 구현합니다. 일반 boto3 요청을
공개 Proxy 엔드포인트로 보내면 됩니다. Catalog 메타데이터의 S3 값은 보통의 `s3://bucket/key` URI로
적고 LocalStack HTTP 엔드포인트를 넣지 않습니다.
...
| `retention` | 5일, snapshot 1개 유지, 만료 파일 정리, 24시간 주기; 주기는 3–168시간 |
...
duplicate, mutation보다 먼저 immutable draft로 parse합니다. Iceberg 대상 여부, 파일 format, 기본
...
종료합니다. History는 설정한 상한까지 카탈로그 schema 3에 보존합니다.

Claim마다 제한 시간이 있는 Glue 5 `spark-submit` process 하나를 실행합니다. Worker는 Apache
...
| retention, 파일 정리 활성 | `expire_snapshots` Spark procedure |
| retention, 파일 정리 비활성 | Iceberg `ExpireSnapshots.cleanExpiredFiles(false)` Java API로 data 파일 삭제 방지 |
| orphan 삭제 | `remove_orphan_files` dry-run 후보를 S3 수정 시각으로 다시 검사해 삭제하며 optimizer 생성 전·생성 시점 파일은 보존 |

Glue는 `sort`, `z-order` 모두 기존 Iceberg sort order를 요구합니다. Emulator는 API 설정 생성은
허용하고 예약 실행이 시작될 때 실제 실행 환경 메타데이터를 검증합니다. Table에 sort order가 없으면
...
Work 파일 mode는 `0600`이며 payload 내용과 credential은 structured log에 복사하지 않습니다.
Boundary log에는 run ID, optimizer type, 제한 시간, 엔드포인트 host, result metric 이름과 수정 안내가
있습니다. Timeout이면 Spark process group 전체를 terminate하고 grace period 뒤 kill합니다. Local mode의 AWS DPU metric은
0이며 가능한 파일/byte/delete count와 duration은 Iceberg 결과에서 얻습니다.
...
- Iceberg procedure 또는 Glue 5 실행 환경: `runtime/table_optimizer_job.py`
- durable format: repository schema migration과 persistence 테스트

세 유형의 실제 Glue 5/Spark 3.5.4/LocalStack 실행은 CI에서만 수행합니다. Local unit 테스트는 fake
executor와 명시적 제한 시간을 사용하며 공개 Proxy boto3 suite는 여섯 API 전부를 실행합니다.
