...
# GlueCatalog을 통한 Iceberg 메타데이터 evolution

<!-- toc:start -->
## 목차

- [책임 경계](#책임-경계)
- [보장하는 evolution 프로필](#보장하는-evolution-프로필)
...
이 계약은 Glue 5.0 상호운용 프로필에서 지원하는 Apache Iceberg 1.7.1 partition, schema, sort,
...
Spark와 Iceberg 1.7.1 실행 환경은 S3에 data 파일, manifest, snapshot, table 메타데이터 JSON,
...
Hidden partitioning 모델에 따라 query는 원본 column을 filter하며 Iceberg가 물리 partition
layout을 caller에게 노출하지 않고 partition value를 유도하고 pruning합니다.

<!-- section: behavior -->
## 보장하는 evolution 프로필

고정된 Glue 5.0/Spark 3.5.4/Iceberg 1.7.1 프로필은 table-driven 실제 실행 환경 시나리오 하나에서
다음 동작을 검증합니다.
...
| Read 동작 | Evolution 전후에 쓴 row를 함께 읽고 원본-column filter가 예상 결과를 반환 |

이 항목은 메타데이터-only Iceberg API 작업입니다. Partition spec이나 schema가 바뀌어도 기존 파일을
...
클라이언트가 만든 table definition을 담은 `UpdateTable`을 받습니다. Version이 맞으면 새 table version
...
`GetTable`과 `GetTableVersions`는 저장한 pointer와 클라이언트가 보낸 column/type 문자열을 모두 반환하고
...
CI 전용 `tests/e2e/test_glue_spark_catalog.py` 시나리오는 공식 Glue 5 image를 공개 Proxy와
LocalStack에 연결합니다. 위 표의 모든 형식을 실행하고 두 schema에서 쓴 row를 읽습니다. boto3로
최종 `metadata_location`을 얻은 뒤 S3 `GetObject`로 실제 메타데이터 JSON을 내려받으며 JSON list
...
저장 경계를 보여줍니다. Spark 또는 Iceberg를 올린 뒤 이 프로필이 깨지면 다음을 확인합니다.

1. Spark DDL이나 실행 환경 동작 변화: `glue/tests/workloads/iceberg_evolution.py`
2. Iceberg 메타데이터 spec 표현 변화: `tests/support/iceberg_metadata.py`
...
5. 고정 실행 환경, 시나리오, 검증 선언: typed pytest compatibility annotation

<!-- section: limits -->
## 한계

Row-level write는 별도 [Iceberg row-level DML 계약](glue-iceberg-row-level-dml.ko.md)이 다룹니다.
Snapshot, reference, 메타데이터 table, procedure 동작은 [snapshot/reference/procedure
