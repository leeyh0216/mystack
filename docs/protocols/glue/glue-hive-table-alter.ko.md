...
Mystack은 Spark와 공식 Glue Hive metastore 클라이언트가 선택한 Glue 카탈로그 요청을 보존하며 Spark
SQL을 직접 parse하지 않습니다. SQL 분석, V1/V2 capability 검사, cache invalidation, SQL 예외는
Spark 책임입니다. `UpdateTable`, 불변 table version, 원자적 카탈로그 persistence는 Mystack
...
| Spark Hive API 작업 | 관찰되는 protocol 경계 | Mystack 보장 |
| --- | --- | --- |
...
| V1 `DROP COLUMN`, `RENAME COLUMN`, `REPLACE COLUMNS`, type 변경 | Spark가 Glue 호출 전에 거부 | 실패 후 카탈로그 state 불변 |
| Hive table `RENAME TO` | 공식 Glue Hive 클라이언트가 `UpdateTable` 전에 거부 | `Table rename is not supported`, 원본 메타데이터 유지 |
...
type 변경은 거부합니다. 공식 Glue Hive 클라이언트의 `alterTable`도 table 이름 변경을 별도로
거부합니다. 이는 클라이언트 소유 결과이므로 Mystack이 비공개 SQL parser를 추가하거나 존재하지
...
현재 Glue 모델은 최상위 `UpdateTable.Name`도 제공합니다. 이는 공식 Hive 클라이언트가 사용하는
rename 경로가 아니라 직접 Glue API surface입니다. `Name`이 원본를 가리키고
`TableInput.Name`이 다르면 Mystack은 table과 모든 카탈로그 partition을 원자적으로 이동합니다.
Source가 없으면 실패하고, 기존 대상은 덮어쓰지 않으며, 대소문자만 다른 변경은 동일한 Hive
...
Catalog 메타데이터 변경은 S3 data를 copy, rename, delete하지 않습니다. Filesystem side effect는
Spark/Hadoop 책임입니다. 권한, lock, 통계 정합성, cache 동작은 emulator 카탈로그 경계 밖입니다.

<!-- section: diagnose -->
## 검증과 진단

실제 port를 사용하는 작은 boto3 계약은 전체 StorageDescriptor/complex column 보존, property,
SerDe, location, 기본 archive, `SkipArchive`, 낙관적 version 검사, 직접 API rename, 대상 충돌,
대소문자 정규화, 원본 부재, partition 보존을 검증합니다. Persistence 계약은 durable save
...
`MYSTACK_E2E_SCENARIO` event를 출력하며 설정된 E2E 제한 시간을 상속합니다.

향후 Spark 또는 Glue 클라이언트 변경으로 깨지면 먼저 범용 AWS dispatcher log에 `UpdateTable`이
있는지 확인합니다. 요청이 없으면 Spark/클라이언트 어댑터 또는 SQL 기대값 변경입니다. Payload가
달라졌으면 inbound table API 작업 family나 botocore 계약을 수정합니다. Payload가 맞지만 state가
틀리면 `TableCommands`, serialization/restart 문제면 repository 어댑터를 확인합니다. Protocol
translation, application policy, persistence 책임을 이 순서로 분리합니다.

<!-- section: exclusions -->
## 제외 범위

Spark V1 Hive 카탈로그에 V2 column API 작업을 추가하거나 Hive lock·권한을 흉내 내거나 managed
