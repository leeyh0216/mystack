...
- [책임 경계](#책임-경계)
- [Rename 순서와 보장](#rename-순서와-보장)
- [Drop과 purge 순서](#drop과-purge-순서)
- [실패와 복구 경계](#실패와-복구-경계)
- [검증 근거](#검증-근거)
...
Apache Iceberg가 SQL을 parse하고 rename과 purge를 여러 system에 걸친 API 작업으로 수행합니다.
...
각 API 작업은 카탈로그 state를 원자적으로 commit합니다. 추적 파일 삭제는 Iceberg S3 FileIO가
담당하고 LocalStack은 설정한 object-store 엔드포인트를 제공합니다.

<!-- section: rename -->
## Rename 순서와 보장

고정한 Iceberg 1.7.1은 대상 namespace를 확인하고 원본를 읽은 뒤, 같은 메타데이터 pointer와
location을 가진 destination Glue table을 만들고 원본를 purge 없이 삭제합니다. Source 삭제가
실패하면 새 destination 삭제를 보상 작업으로 시도합니다. 이는 하나의 원자적 Glue request가 아니라
문서화한 호출 순서입니다.

실제 실행 환경 시나리오는 다음을 보장합니다.
...
| 같은 정규화 이름 또는 case-only 대상 | Spark/Iceberg가 거부하며 두 번째 논리 table이 생기지 않음 |
| Source 또는 대상 namespace 없음 | 실패하고 destination을 공개하지 않음 |
...
Mystack은 카탈로그 identifier를 소문자로 정규화합니다. 빠른 계약은 `events`와 `EVENTS`가
persistence 전에 충돌하고 누락 read가 HTTP 400 `EntityNotFoundException`을 반환함을 증명합니다.
오류는 Mystack 내부의 결정적 유효성 검사 및 API 작업 순서로 평가하며 실제 AWS 계정과 비교하지
않습니다.
...
- `DROP TABLE`: Glue `DeleteTable`만 호출하며 table data와 메타데이터 object를 유지합니다.
- `DROP TABLE ... PURGE`: current Iceberg 메타데이터를 읽고 Glue `DeleteTable`을 먼저 호출한 뒤
  `CatalogUtil.dropTableData`로 해당 메타데이터가 추적하는 파일을 삭제합니다.

E2E 시나리오는 추적 data/메타데이터와 함께 각 table prefix 안에 추적하지 않는 sentinel을
...
prefix 삭제를 구현하지 않았음을 보여줍니다. `IF EXISTS` retry는 Spark/Iceberg가 받아들이며 카탈로그
state를 다시 만들지 않습니다.

<!-- section: failure -->
## 실패와 복구 경계

Rename 보상은 여러 Glue 호출 사이의 best effort이며, 각 Mystack 호출은 원자적 카탈로그
transaction입니다. Source `DeleteTable` 실패 시 Iceberg 보상 삭제가 destination을 제거할 때까지
원본와 destination이 모두 보입니다. Source는 일부만 삭제되는 상태가 되지 않습니다.

Purge는 의도적으로 Glue와 S3 전체에서 transactional하지 않습니다. 고정한 구현은 Glue entry를
추적 S3 파일보다 먼저 삭제합니다. 이후 S3 삭제가 실패하면 SQL `IF EXISTS` retry는 사라진 카탈로그
pointer를 다시 읽을 수 없습니다. 따라서 이전에 확보한 메타데이터 location 또는 별도로 제어하는
orphan-파일 cleanup으로 복구해야 합니다. Mystack은 Iceberg보다 강한 원자성을 주장하지 않습니다.
...
실행합니다. `tests/e2e/test_glue_spark_catalog.py`는 공개 Proxy와 S3 엔드포인트를 통해 결과 Glue
definition을 boto3로 확인하고 object 존재 여부를 검증합니다. Spark process와 테스트 모두 설정한
E2E 제한 시간을 사용합니다.
...
`glue.repository.persistence.*` event는 table document나 object 내용을 노출하지 않으면서 API 작업,
resource fingerprint, side-effect phase, rollback, 수정 hint를 식별합니다.

Upgrade 후 이 프로필이 깨지면 다음을 확인합니다.
...
이 프로필은 Glue와 S3 전체의 rename/purge 원자성, 예약 optimizer 서비스, 카탈로그 삭제 이후 S3
