...
비교 연산자는 `=`, `==`(SDK 클라이언트 호환 alias), `<>`, `!=`, `>`, `>=`, `<`, `<=`를 지원합니다.
...
value는 없습니다. 다른 카탈로그/table/expression/segment의 token은 거부됩니다. 일반
...
streaming으로 최대 `fallback_max_candidates` row만 평가하며 카탈로그 목록을 snapshot하지 않습니다. cap을
...
`fallback_max_candidates`, `supported_key_types`로 resource limit과 호환 프로필을 제어합니다. 기본 길이는 공식 API 모델과
같고 token 상한은 로컬 denial-of-서비스 방어입니다. 구조화 event
...
클라이언트, Glue API 변경으로 pruning이 깨지면 이 event를 먼저 확인하고
`glue/grammar/GluePartitionExpression.g4`, parse-tree 어댑터/evaluator, 격리된 SQLite
compiler/projection module을 수정합니다. `tools/antlr/glue-partition-expression.lock.json`은 generator
URL, version, digest, 제한 시간을 고정합니다. `make antlr-generate`로 생성 코드를 갱신하고 CI의
`make antlr-check`가 drift를 차단합니다.

<!-- section: verification -->
## 검증

빠른 unit 테스트는 SQLite result page와 evaluator를 비교하고, keyset continuation, token scope, order,
typed projection, `LIKE`, segment union/disjointness, 오류 순서를 검증합니다. 실제 port의 boto3
계약는 typed filtering과 `NextToken`, `Segment`를 함께 검증합니다. CI 전용 Glue 5 Spark
시나리오는 S3에 partitioned Hive table을 만들고 partition을 insert한 뒤 Glue Hive metastore 클라이언트를
통한 pruning query를 검증합니다. 모든 테스트 process는 `config/runtime/mystack.yaml`의 deadline을 사용합니다.
