...
이 문서는 Apache Iceberg 1.7.1이 Mystack Glue emulator에 commit할 때 사용하는 카탈로그 pointer
부분을 정의합니다. AWS Glue 5.0에는 Iceberg 1.7.1이 포함되며 기본적으로 optimistic locking을
사용합니다. Iceberg AWS integration은 Glue table `VersionId`로 오래된 메타데이터 pointer 교체를
...
Mystack은 Glue 카탈로그 API 작업을 구현하며 Iceberg table format을 직접 구현하지 않습니다.
Spark/Iceberg가 data, manifest, 메타데이터 JSON, snapshot과 S3 object를 Iceberg 구현으로 생성합니다.
Mystack은 `UpdateTable`에 들어온 `TableInput`과 `Parameters.metadata_location`을 저장하고 요청의
`VersionId`가 현재 값일 때만 current 카탈로그 pointer를 원자적으로 바꿉니다. Iceberg 메타데이터
파일을 parse하거나 다시 쓰지 않습니다.

이 분리는 writer가 독립 메타데이터를 준비한 뒤 현재 메타데이터 pointer를 원자적으로 교체한다는
...
1. AWS JSON 1.1 경계가 모델에 따른 `UpdateTable` 요청을 검증합니다.
2. Short-lived SQLite connection이 설정한 foreign key, journal, synchronous, busy-제한 시간 policy를
...
5. Adapter가 이전 `VersionId`를 조건으로 current table을 update하고, 같은 transaction에서 archive와
   partition-key row를 쓰며 진단용 카탈로그 revision을 증가시킨 뒤 commit합니다.
...
하나를 만듭니다. Iceberg 클라이언트는 새 pointer를 refresh하고 자기 변경을 retry할 수 있습니다. 이는
카탈로그 compare-and-swap이며 S3 data 파일 전체에 대한 global transaction isolation은 아닙니다.

<!-- section: configuration -->
## SQLite transaction 설정

`glue.sqlite.database_file`은 유일한 영속 카탈로그 path입니다. WAL이 `-wal`, `-shm` sibling을
유지하므로 parent directory 전체를 write 가능하게 mount해야 합니다. `busy_timeout_milliseconds`는
busy writer wait 하나의 상한이고 `retry_limit`은 추가 short retry 횟수의 상한입니다. 경합 제한 시간은
...
checkpoint policy, mounted 설정 전체는 [Glue SQLite 실행 환경
...
`.failed`는 expected/current/candidate version과 resource·메타데이터 location의 SHA-256 앞부분만
...
`.transaction.commit.*`, `.transaction.rolled_back`는 카탈로그 storage 경계를 보여줍니다.

Spark/Iceberg 클라이언트 변경으로 이 경로가 깨지면 다음 순서로 확인합니다.

1. Wire member 변경은 고정 botocore 모델 경계와 `glue/adapters/inbound/aws_table.py`
...
5. 실제 클라이언트 retry 변화는 `glue/tests/workloads/iceberg_contention_job.py`와 생성된 compatibility case
...
3.5.4/Iceberg 1.7.1
writer를 공개 Proxy로 실행하고 retry된 append 둘이 모두 보존되는지 확인합니다. One-off container
