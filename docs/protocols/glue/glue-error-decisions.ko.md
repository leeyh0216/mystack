...
Mystack은 공개 API 문서, 고정한 botocore 모델, 내부 카탈로그 invariant로 Glue 오류를 정의하며 실
AWS 계정을 조회하지 않습니다. Source of truth는 `contracts/glue-error-conditions.yaml`입니다.
`scripts/compatibility/glue_error_contracts.py`는 구현한 28개 API 작업 모두에 순서가 있는 계약을 요구하고
[영문](../../compatibility/api-coverage.md)과
[한글](../../compatibility/api-coverage.md) 표를 생성합니다.
...
1. AWS JSON/공식 API 작업 요청 구조와 JSON type
2. Model의 필수 field
...
Protocol 검증은 Glue dispatcher 전의 공통 AWS 엔드포인트에서 수행합니다. 유효한 요청은
...
공통 AWS JSON 1.1 controller는 `__type`, `Message`, 모델 기반 HTTP status,
...
SigV4 메타데이터는 routing context에 사용할 수 있지만 security boundary가 아닙니다.
...
ID, 한 API 작업의 복수 rule, 알 수 없는 API 작업, 인증·인가 오류는 process 시작을
실패시킵니다.

Injection은 API 작업 전체에 적용하고 의도적으로 state가 없습니다. 설정을 끄고 process를
재시작할 때까지 모든 유효한 일치 요청이 실패합니다. 숨은 counter, request header, payload 값
matching이 없어 테스트가 재현 가능합니다. 잘못된 요청 구조는 fault보다 먼저 실패하며 application과
repository side effect는 시작하지 않습니다.

<!-- section: logging -->
## Logging과 유지보수

Operation 수준에서 변환되거나 주입된 오류마다 `glue.error.decision`이 request/API 작업/family, condition ID,
category, phase, error code, HTTP status, mutation 보장, 안전한 failure type을 기록합니다. 공통
dispatcher와 controller는 모델 fingerprint, duration, 최종 code를 추가합니다. Fault는 rule ID를
기록하지만 설정한 message는 기록하지 않습니다.

클라이언트 upgrade로 깨지면 다음 순서로 확인합니다.

1. `protocol.validation.failed`: 고정한 botocore input 모델 변경입니다. 공통 요청 구조 지원 또는
   API 작업 mapping을 수정합니다.
2. `adapter.mapping_failure`: 모델을 통과한 요청이 오래된 inbound family에 도달했습니다.
   Repository가 아니라 해당 family를 수정합니다.
3. Domain condition: application invariant 또는 error 카탈로그 row 변경입니다.
4. `persistence.side_effect_failed`: repository transaction 전후 log와 mounted state path를
   확인합니다.
5. Generated drift: 검토할 API 작업과 condition을 식별합니다.

의도적으로 카탈로그를 바꾼 뒤 `make glue-errors-generate`, 로컬/CI 검증에는
`make glue-errors-check`를 사용합니다.

<!-- section: exclusions -->
## 제외 범위

문서화되지 않은 서비스 bug를 재현하거나 실 AWS와 비교하거나 부분 disk corruption 또는
확률·latency 기반 chaos를 주입하지 않습니다. Glue Job과 Crawler는 구현 API 작업이 아니므로
rule에서 선택할 수 없습니다.
