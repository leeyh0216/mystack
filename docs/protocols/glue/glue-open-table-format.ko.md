...
1.43.66 model을 기준으로 합니다. Mystack은 실제 AWS 계정을 조회하지 않습니다.

<!-- section: responsibility -->
## 책임 경계

일반 Iceberg GlueCatalog 경로에서는 Apache Iceberg가 이미 작성한 metadata pointer를 받아 Mystack이
무손실로 저장합니다. Open Table Format 입력은 다릅니다. AWS가 catalog service에서 materialize할
schema, partition, sort, location, property document를 정의하므로 Mystack이 이를 검증하고 최초 또는
변경된 Iceberg v2 metadata JSON을 작성합니다. Data file, manifest, snapshot, DML, retry와 이후 client
...
Domain은 boto3, S3, FastAPI, repository 구현을 import하지 않습니다. S3 adapter는 catalog state에
접근하지 않습니다.
...
최초 metadata에는 Iceberg v2 UUID, location, schema/current schema, partition specs/default spec,
...
optional struct/list/map 아래 경로를 거부하고 field ID는 Iceberg metadata column 예약 범위보다
...
지원하며 source ID, field ID, name을 검증합니다. Write order는 같은 transform, asc/desc, 두 null
...
기존 Iceberg metadata pointer가 필요합니다. Mystack은 current JSON을 읽고 snapshot과 알지 못하는
spec member를 보존한 뒤 transition을 적용하고 previous metadata entry를 추가합니다. 고유한 다음
...
`remove-properties`입니다. Action이 없으면 전달한 state member를 replace/upsert합니다. Glue 5.0 local
Iceberg profile은 metadata encryption을 구성하지 않으므로 `add-encryption-key`와
`remove-encryption-key`는 결정적으로 `InvalidInputException`을 반환합니다.

S3와 catalog storage는 하나의 distributed transaction을 만들 수 없습니다. Mystack은 preflight,
고유 candidate name, catalog CAS, best-effort 삭제를 제공합니다. 보상 삭제 실패는 미참조 JSON을
남길 수 있지만 일부만 적용된 catalog definition은 공개하지 않습니다.
...
`ConcurrentModificationException`, 설정한 S3/catalog persistence 실패는
...
5. 실제 client 근거: `glue/tests/workloads/open_table_format.py`, typed pytest compatibility annotation

<!-- section: evidence -->
## 검증 근거와 한계

빠른 test는 nested type, global ID, 모든 layer 경계, 공식 boto3 serialization, modeled error,
...
동일한 고정 boto3 model을 검증합니다. Upstream Glue image가 system interpreter를 미리 설정할 수
