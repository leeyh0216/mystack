...
Mystack은 아래 13개 database, table, table-version, import-status API 작업에 대해 결정적인 로컬
카탈로그 판단을 구현합니다. 고정한 botocore 모델이 공개 요청 구조를 먼저 검증합니다. 저장된
schema가 필요하지 않은 application 값은 카탈로그 조회 전에 검증합니다. 이는 공개 문서에 근거한
...
| `UpdateDatabase` | Source/candidate 이름 → 원본 존재 → destination 충돌 → durable save |
| `DeleteDatabase` | 이름 → database 존재 → atomic 카탈로그 cascade → durable save |
...
| `UpdateTable` | Source/candidate 이름과 숫자 `VersionId` → 원본 존재 → rename 충돌 → stale version → archive/mutation → durable save |
...
| `GetCatalogImportStatus` | 성공 또는 설정한 제한 시간/internal failure |

잘못된 요청이나 실패한 candidate는 visible/durable state를 변경하지 않습니다. 중복 create와 rename
destination은 `AlreadyExistsException`, 없는 카탈로그 resource는 `EntityNotFoundException`,
application 검증은 `InvalidInputException`입니다. Stale `UpdateTable.VersionId`는 공식
`UpdateTable` API 작업에 모델된 `ConcurrentModificationException`입니다. 전역 Glue exception인
`VersionMismatchException`은 `UpdateTable` 모델 오류가 아니므로 이 API 작업에서 노출하지 않습니다.
...
번의 로컬 durable commit으로 database/table과 child를 접근 불가능하게 합니다. 문서화된 delete 후
...
cross-account/cross-Region, 인증·인가는 로컬 trigger가 없고 범위 밖입니다. 이 release는 인위적인
카탈로그 quota를 정의하지 않으므로 `ResourceNumberLimitExceededException`의 자연 trigger가 없습니다.
지원 카탈로그 mutation에는 비동기 로컬 resource 상태가 없으므로 `ResourceNotReadyException`을 만들지
않습니다. 설정 system fault는 제한 시간/internal만 다루며 제외한 상태를 가장하지 않습니다.

<!-- section: verification -->
## 검증과 유지보수

`glue/tests/test_database_table_error_semantics.py`는 parameterized AWS JSON boundary 계약을 실행하고
실패 API 작업 전후 durable store snapshot을 비교합니다. Projection, archive, rename, cascade,
persistence rollback도 검증합니다. 로컬에서는 설정 제한 시간으로 이 제한된 suite만 실행하고 공개
Proxy/boto3 범위는 CI가 담당합니다. 요청 구조/projection drift는 inbound API 작업 family, 결정 순서
