...
이 계약은 Glue 5.0, Spark 3.5.4, Iceberg 1.7.1 프로필에서 지원하는 row-level write 동작을
...
파일, manifest, snapshot summary, table 메타데이터를 S3에 만듭니다. Mystack은 SQL을 parse하거나 row를
평가하지 않고 copy-on-write와 merge-on-read도 구현하지 않습니다. 클라이언트가 만든 Glue
...
| Copy-on-write | `UPDATE`, `DELETE FROM`, matched/not-matched `MERGE INTO` action이 예상 row를 만들고 현재 delete 파일 수는 0 |
| Merge-on-read | 같은 row-level mutation 계열이 예상 row를 만들고 실제 Iceberg v2 delete-파일 근거를 유지 |
| Merge 유효성 검사 | 한 대상 row와 일치하는 원본 row가 두 개이면 Iceberg Spark write 계약에 따라 실패 |
| 실패한 commit | 실패한 merge는 Glue version과 Iceberg snapshot을 만들지 않고 이전 row와 pointer를 current로 유지 |
| 경합 | 결정적인 stale-pointer 계약이 losing candidate를 거부하고 기존 두-container 테스트가 같은 Glue CAS 경로의 Iceberg refresh/retry를 검증 |

`write.delete.mode`, `write.update.mode`, `write.merge.mode`를 `copy-on-write` 또는
`merge-on-read`로 명시합니다. 클라이언트의 암묵적 기본값은 검증 근거로 사용하지 않습니다.
Merge-on-read delete 파일은 v2 기능이므로 table format version은 2입니다.

<!-- section: wire -->
## Glue wire와 실패 계약

성공한 각 row-level Iceberg commit은 이전 `VersionId`와 다음 메타데이터 pointer를 담은 modeled Glue
...
따릅니다. Glue commit 성공 이후의 row/파일 의미론은 계속 Iceberg가 소유합니다.
...
정확한 current pointer, 모든 table version에서 losing 메타데이터 location이 없음을 확인합니다.

CI 전용 `tests/e2e/test_glue_spark_catalog.py` 시나리오는 공식 Glue 5 image에서 공개 Proxy와
LocalStack을 사용합니다. 중요 경계 이후 row를 확인하고 boto3로 최종 pointer를 읽은 뒤 S3에서 실제
table 메타데이터 JSON을 내려받습니다. Format version, write-mode property, snapshot 수, current snapshot,
전체 delete-파일 근거를 검증합니다. 모든 process/테스트 wait는 compatibility case의 명시적 제한 시간
안에서 실행합니다.

<!-- section: observability -->
## Logging과 수정 위치

각 DML 경계는 `MYSTACK_E2E_SCENARIO`로 시나리오 이름, phase, 안전한 exception class만 출력합니다.
...
클라이언트를 올린 뒤 이 프로필이 깨지면 다음을 확인합니다.

1. Spark SQL 또는 Iceberg write-mode 변화: `glue/tests/workloads/iceberg_row_level.py`
2. Iceberg snapshot-summary 표현 변화: `tests/support/iceberg_metadata.py`
...
5. 고정 프로필과 시나리오 변경: typed pytest compatibility annotation

<!-- section: limits -->
## 한계

Snapshot time travel, branch/tag write, 메타데이터 table, procedure는
