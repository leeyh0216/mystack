...
이 운영자 전용 hook을 사용하세요. EMR 서비스, bootstrap action과 Spark Step보다 먼저 실행합니다.
기본값은 비활성이며 EMR container에만 적용됩니다.

<!-- section: quick-start -->
## 검토한 script로 게시 image 시작하기

같은 release tag에서 기본 Compose 파일, overlay와 예제 script를 받으세요. 시작 전에 placeholder
...
```bash
export MYSTACK_IMAGE_TAG=v0.1.6  # 실제 게시 tag로 교체
...
파일이나 검토하지 않은 script를 이 directory에 두지 마세요.

<!-- section: lifecycle -->
## 실행 순서와 사용자 계약

Container entrypoint는 `root`로 시작해 byte 기준 파일 이름 순서로 `*.sh`를 한 번 찾고 같은 shell에서
각 script를 원본합니다. 하나라도 0이 아닌 값으로 끝나면 container를 즉시 중지하며 뒤 script와
EMR 서비스를 실행하지 않습니다. 따라서 export한 환경변수는 EMR API process, `hadoop` bootstrap
...
directory, group 또는 다른 사용자가 쓸 수 있는 directory, 안전하지 않은 이름, symlink, 일반 파일이
...
`10-ca.sh`, `20-environment.sh`처럼 안정적인 숫자 prefix를 사용하세요. Mount한 파일을 바꾸면 container를
...
`update-ca-trust extract`를 실행하세요. 클라이언트가 명시적 경로를 요구하면 `SSL_CERT_FILE`,
...
이 정보는 실제 base OS, 서비스 UID/GID/home, 확인된 실행 파일과 version, Python package와 CA
...
Java 또는 Spark 갱신 뒤 문제가 생긴 파일을 찾을 수 있습니다.
...
검증 범위는 EMR 서비스 전 한 번 실행하는 신뢰된 lexical order 원본, 즉시 실패 중단, 권한 전환을
통과한 환경 전달, 최종 `hadoop` 사용자, signal-safe PID 1 교체, 실행 환경 정보와 게시 GHCR image의
