...
Mystack은 health 엔드포인트가 준비되기 전에 검토된 process-로컬 EMR cluster 목록을 생성할 수
있습니다. 이 기능은 emulator 배포 입력이며 새로운 AWS API 작업이 아닙니다. 각 `clusters`
...
`StepConcurrencyLevel`, `Tags`입니다. 상위 botocore 모델에 member가 있어도 Mystack이
emulation하지 않으면 거부할 수 있습니다. 유효해 보이는 파일에서 사용자의 의도를 조용히
...
첫 cluster를 만들기 전에 YAML 전체를 parse하고 모든 entry를 고정 botocore `RunJobFlow` 모델,
구현 member allowlist, 설정한 release 프로필, 중복 이름, Step limit으로 검증합니다. 하나라도
실패하면 EMR HTTP server가 healthy 상태가 되지 않습니다. 일부만 검증된 plan은 실행하지 않습니다.

검증 후 inbound 파일 어댑터가 entry를 기술 독립적인 `CreateCluster` command로 mapping하고 파일
...
`DescribeCluster`, management resource 엔드포인트와 Console이 같은 aggregate를 봅니다.

현재 EMR repository는 process-로컬입니다. EMR container를 다시 시작하면 새 cluster ID로 설정
목록을 다시 만들며 이전 ID를 보존하거나 이름을 reconcile하지 않습니다. 결정적인 파일
...
Mounted main 설정의 `emr.startup_clusters_file`을 지정해도 됩니다. Relative path는 main
설정 파일 옆을 기준으로 해석합니다. Nested environment override는
`MYSTACK__EMR__STARTUP_CLUSTERS_FILE`이며 `null`이면 비활성화됩니다. External 파일 수정 후 EMR을
...
secret은 기록하지 않고 원본, fingerprint, definition index, 개수, cluster identity, 수정 위치를
남깁니다. 새 boto3/botocore 입력이 검증되지 않으면 고정 모델 manifest와 generic validator를 먼저
