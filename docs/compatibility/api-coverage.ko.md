...
- [개요](#개요)
- [정책](#정책)
- [현재 구현한 operation](#현재-구현한-operation)
...
서비스 단위 API index는 [Amazon EMR API operation](https://docs.aws.amazon.com/emr/latest/APIReference/API_Operations.html)과
[AWS Glue Web API operation](https://docs.aws.amazon.com/glue/latest/webapi/API_Operations.html)입니다.
...
model만 추적한다는 뜻입니다. Glue의 `NOT_PLANNED`는 Job, JobRun, Crawler입니다. Commit한 JSON
기준선은 각 작업의 상태와 데이터 구조 지문을 저장합니다. `--check`는 새로운 상위 작업에 기본
상태를 자동 부여하지 않고 미분류로 보고해 CI를 실패시킵니다. 데이터 구조 변경과 삭제도 adapter,
test, 문서 수정 안내와 별도로 보고합니다.
