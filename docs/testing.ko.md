...
| 계층 | 목적 | 외부 실행 환경 | Timeout 출처 |
| --- | --- | --- | --- |
| Unit | Domain 상태, codec, routing, 설정 | 없음 | `tests.unit_timeout_seconds` |
| Architecture | 안쪽 import와 도메인 경계 | 없음 | unit 제한 시간 |
| Frontend | 공통 theme/primitive, React component, TypeScript, production asset | Node/jsdom | `MYSTACK_FRONTEND_TEST_TIMEOUT_MS`와 CI job 제한 시간 |
...
모든 pytest 실행은 thread 방식의 `pytest-timeout`을 사용해 hang 시 Python thread stack을 출력합니다. Spark/bootstrap 어댑터도 YAML의 서비스별 process 제한 시간을 받습니다. Vitest는 테스트와 hook에 명시적으로 설정 가능한 millisecond deadline을 사용하며 CI는 lint, typecheck, 테스트, production build 전체를 job 제한 시간으로 한 번 더 제한합니다.

<!-- section: contracts -->
## Contract 규칙

- boto3는 공개 Proxy 엔드포인트에만 연결합니다.
- 성공 결과와 modeled AWS error code, HTTP status, side effect를 함께 검증합니다.
- 구현된 모든 API 작업은 boto3 범위가 필요합니다.
...
- EMR lifecycle 테스트는 부분 startup과 driver 실패를 주입하고 scheduler를 두 번 닫으며 실제 child
  process를 실행해 역순 cleanup, task/process/lock 무누수, deadline 사용을 검증합니다. 책임 테스트는
  cluster-command, Step-command, query handler의 공개 surface도 고정합니다.
- Operation-family 테스트는 서비스 어댑터 없이 모든 EMR·Glue family를 생성해 소유권이 겹치지
  않는지, 그 합집합이 구현 호환성 범위와 양방향으로 같은지, family-로컬 modeled error 변환이
  유지되는지 확인합니다. 공통 registry mutation 테스트는 중복·누락·예상 밖 handler를 request
...
- Read-only versioned cluster 파일로 image를 시작하고 `RunJobFlow`를 호출하지 않은 상태에서 boto3와
  management 경계로 cluster를 찾습니다. EMR 재시작 뒤에는 새 ID 하나만 존재해야 합니다. Unit
  계약는 entry 하나가 잘못되면 command를 호출하기 전에 plan 전체를 거부합니다.
- Read-only pre-start overlay로 EMR을 시작해 일회용 CA를 OS/Python과 복사한 Java truststore에
  ...
  전달되는지 검증합니다. 별도 raw-container 계약는 즉시 실패 exit code 보존, 뒤 script 미실행,
...
- 실제 Python 및 Java JAR Spark 3.5.x application의 S3A output과 Step 상태, 주/dependency
  산출물 materialize, 실행 중 subprocess 취소를 검증합니다. JAR 제출은 Spark 공식
...
- 성공, 준비 실패, 취소 Step이 정확한 gzip Step/application key 집합을 LocalStack S3에
  게시하는지 공개 Proxy로 검증하고 management API의 publication 증거와 일치하는지 확인합니다.
  배치는 [공식 EMR log 경로](https://docs.aws.amazon.com/emr/latest/ManagementGuide/emr-plan-debugging.html)를 따릅니다.
- 구현된 EMR 13개와 Glue 28개 API 작업 전부를 공개 Proxy 경계로 검증하며, 같은
  재사용 Glue 시나리오를 Glue 서비스 직접 경계에서도 실행합니다.
- boto3와 Spark Hive/Iceberg 어댑터로 Glue Catalog를 검증합니다.
...
- Glue state store의 실패·cancellation·같은 process 및 spawn process concurrent writer·stale table
  version·상한이 있는 파일-lock contention·restart·rename/cascade·schema-1 migration을 주입합니다. 이
  계약은 Data Catalog 메타데이터 원자성과 Iceberg 카탈로그-pointer CAS를 검증하지만 Mystack이 Iceberg
  data/manifest/snapshot 기능을 구현한다는 뜻은 아닙니다.
- Glue domain 생성 전후 input/output dictionary를 변경해도 name, table revision/archive/CAS,
  partition cardinality, aggregate move가 immutable인지 확인합니다. 실행 가능한 책임 테스트는 각
  handler와 repository의 공개 method를 문서화된 범위로 제한합니다.
...
- Managed Iceberg optimizer 세 유형을 공개 boto3 API와 실제 Glue 5 scheduler/Spark worker로
  ...
  기준으로 합니다.
- Playwright로 Proxy를 경유한 두 서비스 소유 React UI를 조작해 EMR cluster 생성·종료, Step 제출·추적·취소·조회,
  ...
  안에서만 사용합니다.
- 긴 Step 실행 중 EMR Compose 서비스를 재시작한 뒤 recovered Console projection, 보존된 stdout,
  boto3 modeled not-found 동작, idempotent S3 archive 재게시를 검증합니다. Compose subprocess와
  모든 HTTP/SDK wait는 설정한 E2E 제한 시간을 사용합니다. 주입한 lifecycle event는 Docker 공식
  [`compose restart` 계약](https://docs.docker.com/reference/cli/docker/compose/restart/)을 따릅니다.
- 현재 Iceberg 시나리오는 create, append, read, 검토한 모든 transform의 hidden partition
  ...
  주요 메타데이터 table, snapshot/maintenance procedure, 범위가 제한된 orphan cleanup, namespace 간
  rename, 카탈로그-only drop, 추적 파일 purge와 서로 다른 Glue-image container의 barrier 동기화
  Spark writer 두 개를 검증합니다. 클라이언트가 stale `VersionId` commit을 refresh/retry하고 append
...
Spark 카탈로그 E2E는 [AWS Glue Libraries](https://github.com/awslabs/aws-glue-libs)가 공개한 Glue 5
실행 환경 프로필(Spark 3.5.4, Python 3.11)을 따르며 Apache Iceberg Java 구현을 기준 integration으로
검증합니다. Catalog discovery 회귀는 생성 뒤 Spark SQL `SHOW TABLES`, `SHOW NAMESPACES`를 실행합니다.
즉 공개 Proxy가 직접 table handle뿐 아니라 Hive metastore와 Iceberg GlueCatalog의 발견 경로도
보존함을 증명합니다. Iceberg upstream project는 이 시나리오에 쓰는 Spark module을 engine integration으로
...
`uv.lock`, `package-lock.json`, hash-locked container export, YAML 실행 환경 프로필, immutable container-base digest,
botocore manifest, Spark checksum/version, Iceberg version은 모두 테스트 입력입니다. 어느
하나를 갱신해도 해당 manifest/프로필 문서와 E2E 증거가 필요합니다. CI는 `uv.lock`과
...
서비스 image가 자기 build asset을 직접 제공하며 Proxy가 안정적인 path를 보존함을 검증합니다.

<!-- section: local-contracts -->
## Local 호환성 기준

Mystack은 동작 비교를 위해 실 AWS 계정을 호출하지 않습니다. 공식 AWS API 문서와 고정
botocore 모델이 API 작업, 데이터 구조, 제약, 선언된 오류를 정의합니다. 여러 잘못된 조건 중 무엇을
먼저 반환할지 문서가 정하지 않으면 검토한 내부 유효성 검사 순서로 최초 실패를 결정합니다.
Parameterized 로컬 계약가 Catalog 상태 오류를 모두 재현하고, 자연스러운 상태 조건이 없는
문서화된 internal/제한 시간 실패는 설정 기반 fault injection으로 재현합니다. 인증·인가 오류는
프로젝트 범위 밖입니다.
[Partition/batch 오류 계약](protocols/glue/glue-partition-batch-errors.ko.md)은 Docker를 시작하지 않고
모델 최댓값, 검증 순서, 안정적인 부분 성공, `UnprocessedKeys`, persistence rollback을 검증합니다.

AWS의 [Hexagonal architecture 모범 사례](https://docs.aws.amazon.com/prescriptive-guidance/latest/hexagonal-architectures/best-practices.html)는 독립 core 테스트와 E2E 자동화를 권장합니다.
