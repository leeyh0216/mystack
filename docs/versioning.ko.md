...
**Actions**에서 **Prepare version PR**을 선택하고 **Run 워크플로**에서 `patch`, `minor`, `major`,
...
permission이 없으므로 게시할 수 없습니다. 이 절차는 GitHub 공식 [수동 워크플로 실행
...
| Event | 결과 | 변경 |
| --- | --- | --- |
| `develop` 또는 `main` 대상 PR | build, 테스트, version 검증 | 없음 |
| `feature/*` push | build와 테스트 | 없음 |
...
권한 없는 CI 워크플로는 `contents: read`만 사용합니다. 게시는 GitHub의 [`workflow_run` 완료
event](https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows#workflow_run)에서만
시작하며 원본 event, 워크플로 name, branch, conclusion, 정확한 head SHA를 다시 검사합니다.
Reusable 워크플로 권한은 GitHub [permission
...
모든 job이 승인된 정확한 SHA를 checkout합니다. Repository 계약와 release acceptance case를
실행한 뒤 로컬 multi-platform build와 고정 Trivy 검사를 수행합니다. Content hash를 가진 aggregate
...
tag 대상 또는 image revision label이 원래 SHA와 같을 때만 이어서 처리합니다. 다른 SHA이면
...
- `feature/*`: 게시 워크플로 trigger와 package write token 없음
...
| `release.policy.failed` | Event, ref, 원본 워크플로가 게시 불가 | 일반 PR을 사용하고 CI에 write 권한을 추가하지 않음 |
...
| 일부 image만 게시 | 같은 SHA의 일부 component만 존재 | 같은 워크플로를 재실행하면 일치하는 image를 검증하고 건너뜀 |
| 익명 검증 거부 | GHCR package가 공개이 아님 | `container-release.ko.md`의 1회성 visibility 절차 후 재실행 |
...
모든 외부 명령에는 제한 시간이 있습니다. 경계 log에는 action, branch, version, component, revision,
복구 안내를 남기지만 token은 기록하지 않습니다. Actions 산출물에서 preflight scan, aggregate
authorization, 게시 index, retention 메타데이터를 확인합니다.
...
서명된 Git tag, snapshot 자동 삭제, 실 AWS 산출물는 게시하지 않습니다. 이 항목을 추가하면
release 계약이 달라지므로 별도 issue, 설정 schema 변경, 테스트, 한·영 문서가 필요합니다.
