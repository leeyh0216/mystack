...
export function StepLogs({api, cluster, step, bufferBytes, onCancel}: {api: EmrUiApi; cluster: EmrCluster; step: EmrStep; bufferBytes: number; onCancel: () => Promise<void>}) {
  ...
  return <div className="grid gap-5">
    <div className="flex min-w-0 flex-wrap items-start justify-between gap-3">
      ...
      <div className="flex flex-wrap items-center gap-2"><Badge tone={complete ? "positive" : paused ? "warning" : "info"}><span id="logFollowStatus">{paused ? "Paused" : status}</span></Badge><Button id="toggleLogFollow" disabled={complete} onClick={() => setPaused(value => !value)}>{paused ? "Resume follow" : "Pause follow"}</Button><Button id="downloadLogs" onClick={download}>Download</Button><Button id="cancelStep" variant="danger" disabled={!ACTIVE.has(snapshot?.step_state || step.state)} onClick={() => void onCancel()}>Cancel Step</Button></div>
    </div>
