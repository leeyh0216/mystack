...
Mystack은 Docker Compose로 로컬 Amazon EMR과 AWS Glue Data Catalog 환경을 실행합니다. AWS 클라이언트,
Spark, LocalStack S3를 사용하는 애플리케이션과 데이터 파이프라인 개발에 사용할 수 있습니다.

<!-- section: start -->
## Docker Compose로 시작하기

게시된 image version을 선택하고 같은 version의 Compose 파일을 받은 뒤 stack을 시작합니다.
...
| 하려는 일 | 시작 문서 |
| --- | --- |
| Docker Compose를 시작하고 AWS 클라이언트 연결 | [시작 안내](docs/getting-started.ko.md) |
| 로컬 AWS SDK for pandas 또는 Spark 클라이언트 lab을 끝까지 실행 | [Client lab](docs/client-workflows.ko.md#해당-lab-실행) |
| boto3, AWS SDK for pandas, Spark Hive, Iceberg로 Glue Data Catalog 사용 | [Glue 안내](docs/glue.ko.md) |
| 클라이언트별 Glue와 EMR 작업 흐름 따라가기 | [Client 작업 흐름](docs/client-workflows.ko.md) |
| EMR cluster 생성, bootstrap action 실행, Spark Step 제출 | [EMR 안내](docs/emr.ko.md) |
| Port, 제한 시간, storage, 실행 환경 설정 변경 | [설정](docs/configuration.ko.md) |
| 지원하는 클라이언트 경로 확인 | [호환성](docs/compatibility/client-matrix.ko.md) |
| EMR·Glue 관리 UI와 진단 사용 | [운영](docs/operations.ko.md) |

<!-- section: contribute -->
## Contributors

Architecture, protocol reference, 개발 환경, 테스트, CI, release 운영은
